...
import torch
from torch.nn import Module, Linear, Conv1d, ConvTranspose1d, BatchNorm1d, ReLU, Sequential
...
def batch_music_spectrum(data, A, K):
...
def batch_find_peaks(pseudospectrum, K):
...
def batch_support_least_squares(data, peaks, A):
...
class LMUSIC(Module):
    def __init__(self, signal_dim, n_filters, n_layers, kernel_size=3, kernel_out=25):
        ...
        self.first_layer = Linear(signal_dim, signal_dim * n_filters, bias=False)
        self.hidden_layers = []
        ...
        self.outer_layer =  Sequential(
            Conv1d(n_filters, 1, kernel_size=kernel_out, padding='same', bias=False),
            ReLU() # don't want negative indicies
        )
